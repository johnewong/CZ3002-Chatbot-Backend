...
//import static java.util.regex.ASCII.BLANK;
...
public class ReadExcel {
     ...
     */
    public static List<RateResponse> readExcel(String fileName) {
        ...
        try {
            ...
            // 读取excel中的数据
            List<RateResponse> resultDataList = parseExcel(workbook);
     ...
     * @return 解析结果
     */
    private static List<RateResponse> parseExcel(Workbook workbook) {
        List<RateResponse> resultDataList = new ArrayList<>();
        // 解析sheet
        for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
            ...
            for (int rowNum = rowStart; rowNum < rowEnd; rowNum++) {
                ...
                RateResponse resultData = convertRowToData(row);
                if (null == resultData) {
     ...
     * @return
     */
    private static Integer convertCellValueToInteger(Cell cell) {
        if (cell == null) {
            return null;
        }
     ...
     */
    private static RateResponse convertRowToData(Row row) {
        RateResponse resultData = new RateResponse();

        Cell cell;
        int cellNum = 0;
        // 获取rateID
        cell = row.getCell(cellNum++);
        Integer rateID = convertCellValueToInteger(cell);
        resultData.setRateResponseId(rateID);
        /*获取年龄
        cell = row.getCell(cellNum++);
